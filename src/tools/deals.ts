...
import { textResult, errorResult, fmt, fmtPrice, stripUndefined, verifyWritePin } from "./helpers.js";
...
export function registerDealTools(server: McpServer, client: PropstackClient): void {
  ...
  server.registerTool(
    ...
    {
      title: "Search Deals",
      description: `Search and filter deals (contact↔property relationships) in Propstack.
...
- Pipeline view: deal_pipeline_id + sort_by=deal_stage_id`,
  ...
  server.registerTool(
    ...
    {
      title: "Create Deal",
      description: `Create a deal linking an interested contact to a property in Propstack.
...
get_pipeline to find valid pipeline and stage IDs.`,
    ...
    async (args) => {
      try {
        ...
        const { write_pin: _, ...dealArgs } = args;
        const [deal, pipelines] = await Promise.all([
          client.post<PropstackDeal>(
            "/client_properties",
            { body: { client_property: stripUndefined(dealArgs) } },
          ),
          fetchPipelines(client).catch(() => [] as PropstackDealPipeline[]),
        ]);
  ...
  server.registerTool(
    ...
    {
      title: "Update Deal",
      description: `Update an existing deal in Propstack.
