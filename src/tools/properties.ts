...
export function registerPropertyTools(server: McpServer, client: PropstackClient): void {
  ...
  server.registerTool(
    "search_properties",
    {
      ...
      description: `Search and filter properties (Objekte) in Propstack CRM.
...
Always returns total count. Use expand=true for custom fields.`,
  ...
  server.registerTool(
    ...
    {
      title: "Get Property",
      description: `Get full details of a single property by ID.
...
Always fetches with new=1 (extra fields) and expand=1 (custom fields).`,
  ...
  server.registerTool(
    "create_property",
    {
      ...
      description: `Create a new property (Objekt) in Propstack CRM.
...
rs_category provides sub-types (e.g. PENTHOUSE, VILLA, MAISONETTE for APARTMENT/HOUSE).`,
    ...
    async (args) => {
      const { status, ...propertyArgs } = args;
      const payload: Record<string, unknown> = { ...propertyArgs };
      if (status !== undefined) payload["property_status_id"] = status;

      const addr = [args.street, args.house_number].filter(Boolean).join(" ");
      const city = [args.zip_code, args.city].filter(Boolean).join(" ");
      const location = [addr, city, args.country].filter(Boolean).join(", ") || "no address";
      const priceStr = args.price ? `${args.price.toLocaleString("de-DE")} €` : args.base_rent ? `${args.base_rent.toLocaleString("de-DE")} €/mo` : "no price";
      const summary =
        `Create property: ${args.title ?? "(no title)"}\n` +
        `  Address: ${location}\n` +
        `  Type: ${args.marketing_type ?? "?"} / ${args.rs_type ?? "?"}\n` +
        `  Price: ${priceStr}`;

      return stageMutation("create_property", summary, async () => {
        const property = await client.post<PropstackProperty>(
          "/units",
          { body: { property: stripUndefined(payload) } },
        );
        // Re-fetch for full data (POST returns minimal object)
        try {
          const full = await client.get<PropstackProperty>(`/units/${property.id}`, { params: { new: 1, expand: 1 } });
          return `Property created (ID: ${property.id}).\n\n${formatProperty(full)}`;
        } catch {
  ...
  server.registerTool(
    "update_property",
    {
      ...
      description: `Update an existing property in Propstack CRM.
...
Use get_property_statuses to look up valid status IDs.`,
    ...
    async (args) => {
      ...
      return stageMutation("update_property", summary, async () => {
  ...
  server.registerTool(
    "get_property_statuses",
    {
      title: "Get Property Statuses",
      description: `List all available property statuses in Propstack.
